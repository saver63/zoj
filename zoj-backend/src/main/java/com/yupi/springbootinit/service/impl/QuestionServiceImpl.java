...
public class QuestionServiceImpl extends ServiceImpl<QuestionMapper, Question> implements QuestionService {
    ...
    public void validQuestion(Question question, boolean add) {
        ...
        if (add) {
            // todo 补充校验规则
            ThrowUtils.throwIf(StringUtils.isAnyBlank(title,tags,content), ErrorCode.PARAMS_ERROR);
        }
        // 修改数据时，有参数则校验
        // todo 补充校验规则
        ...
        if (StringUtils.isNotBlank(answer) && answer.length() > 8192) {
            ThrowUtils.throwIf(answer.length() > 80, ErrorCode.PARAMS_ERROR, "标题过长");
        }
        if (StringUtils.isNotBlank(judgeCase) && judgeCase.length() > 8192) {
            ThrowUtils.throwIf(judgeCase.length() > 80, ErrorCode.PARAMS_ERROR, "判题用例过长");
        }
